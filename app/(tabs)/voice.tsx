...
function ChoiceChip({ label, active, color, onPress, chipBg, chipBorder, chipText }: {
  ...
  return (
    <Pressable
      onPressIn={() => { scale.value = withSpring(0.90, { damping: 20, stiffness: 380 }); }}
...
function SaveBtn({ onPress, label, color, disabled, textColor }: {
  onPress: () => void; label: string; color: string; disabled: boolean; textColor: string;
}) {
  const scale = useSharedValue(1);
  const anim  = useAnimatedStyle(() => ({ transform: [{ scale: scale.value }] }));
  return (
    ...
    >
      <Animated.View style={[s.saveBtn, { backgroundColor: color, opacity: disabled ? 0.6 : 1 }, anim]}>
        <Text style={[s.saveText, { color: textColor }]}>{label}</Text>
      </Animated.View>
    </Pressable>
  );
}
...
export default function VoiceScreen() {
  ...
  const start = async () => {
    try {
      ...
      const { recording } = await Audio.Recording.createAsync({
        ...
        ios: { extension: '.m4a', outputFormat: Audio.IOSOutputFormat.MPEG4AAC, audioQuality: Audio.IOSAudioQuality.MEDIUM, sampleRate: 44100, numberOfChannels: 1, bitRate: 128000, linearPCMBitDepth: 16, linearPCMIsBigEndian: false, linearPCMIsFloat: false },
        web: { mimeType: 'audio/webm', bitsPerSecond: 128000 },
        isMeteringEnabled: true,
      });
      recRef.current = recording; samplesRef.current = [];
  ...
  const stop = async () => {
    ...
    try {
      ...
      setResult({ audioPath: uri, duration, emotion, modelEmotion: emotion, confidence, features, desc: featureDescription(features), modelVersion: modelVersion ?? 'veil-engine-v1' });
      setPhase('done');
    } catch (err) { console.error(err); setPhase('idle'); }
  };

  useEffect(() => () => {
    clearInterval(timerRef.current!);
    if (recRef.current) { recRef.current.stopAndUnloadAsync().catch(() => {}); recRef.current = null; }
  }, []);
  ...
  const saveVoiceResult = async () => {
    ...
    setSaving(true);
    try { await addVoiceEntry(result.audioPath, result.emotion, result.modelEmotion, result.confidence, result.features, result.duration, result.modelVersion); }
    finally { setSaving(false); setResult(null); setPhase('idle'); }
  };

  const onMic      = () => { if (phase === 'idle' || phase === 'done') start(); else if (phase === 'recording') stop(); };
  const micColor   = phase === 'recording' ? '#FF6B6B' : t.accent;
  const ringBorder = phase === 'recording' ? 'rgba(255,107,107,0.35)' : t.accent + '44';
        ...
        <View style={s.micArea}>
          <Pressable onPress={onMic} disabled={phase === 'processing'}>
            <Animated.View style={[s.ring, { borderColor: ringBorder }, ringStyle]}>
              <View style={[s.micBtn, { backgroundColor: micColor }]}>
                {phase === 'processing'
                  ? <ActivityIndicator color={t.textOnAccent} size="large" />
          ...
          {phase === 'recording' && (
            <>
              <View style={s.recRow}>
                <View style={s.recDot} />
                <Text style={[s.recTime, { color: t.textMuted }]}>{secStr}</Text>
              </View>
              <View style={s.wave}>
                {(waveBars.length > 0 ? waveBars : Array(20).fill(0.1)).map((a, i) => (
                  <View key={i} style={[s.waveBar, { backgroundColor: t.accent, height: Math.max(4, a * 44), opacity: 0.3 + (i / Math.max(waveBars.length, 1)) * 0.7 }]} />
                ))}
              </View>
            </>
          )}
...
const s = StyleSheet.create({
  ...
  hint:        { fontSize: 14 },
  resultCard:  { borderRadius: 18, borderWidth: 0.5, padding: 22, alignItems: 'center', width: '85%', gap: 8 },
  resultBadge: { fontSize: 12 },
  resultEmo:   { fontSize: 26, fontWeight: '700' },
  resultDesc:  { fontSize: 14, textAlign: 'center' },
  choiceGrid:  { flexDirection: 'row', flexWrap: 'wrap', justifyContent: 'center', gap: 7, marginTop: 4 },
  chip:        { borderRadius: 14, borderWidth: 1, paddingHorizontal: 10, paddingVertical: 5 },
  chipText:    { fontSize: 12, fontWeight: '600' },
  pillRow:     { flexDirection: 'row', gap: 10, marginTop: 4 },
  pill:        { borderRadius: 10, paddingHorizontal: 12, paddingVertical: 6, alignItems: 'center' },
  pillLabel:   { fontSize: 10 },
  pillVal:     { fontSize: 14, fontWeight: '600', marginTop: 2 },
  privacy:     { fontSize: 11 },
  ...
  recentLabel: { fontSize: 11, fontWeight: '600', letterSpacing: 0.07, textTransform: 'uppercase', marginBottom: 10 },
  entryRow:    { flexDirection: 'row', alignItems: 'center', gap: 12, paddingVertical: 10, borderTopWidth: 0.5 },
